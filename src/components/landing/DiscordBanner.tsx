...
const DiscordBanner = () => (
  <section className="py-16 px-4">
    ...
    >
      ...
      <motion.div className="relative z-10" initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: 0.2, duration: 0.5 }}>
        <motion.div className="inline-flex items-center justify-center w-14 h-14 rounded-2xl bg-[hsl(235_86%_65%/0.15)] mb-5" animate={{ y: [0, -6, 0] }} transition={{ duration: 3, repeat: Infinity, ease: "easeInOut" }}>
        ...
        <motion.a href={DISCORD_LINK} target="_blank" rel="noopener noreferrer" whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }}>
          <Button size="lg" className="glow-primary gap-2 px-8 text-sm font-bold h-12 rounded-xl bg-[hsl(235_86%_65%)] hover:bg-[hsl(235_86%_60%)] text-white">
            <ExternalLink className="h-4 w-4" /> Join Discord Server
          </Button>
        </motion.a>
      </motion.div>
    </motion.div>
