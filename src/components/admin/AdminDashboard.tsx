import { useState, useEffect } from "react";
import { Card, CardContent, CardHeader, CardTitle } from "@/components/ui/card";
import { Badge } from "@/components/ui/badge";
import { getAllTickets } from "@/lib/storage";
import type { Ticket } from "@/lib/storage";
import { Ticket as TicketIcon, Clock, CheckCircle2, Eye, AlertTriangle, TrendingUp, MessageCircle } from "lucide-react";

const AdminDashboard = () => {
  const [tickets, setTickets] = useState<Ticket[]>([]);
  const [loading, setLoading] = useState(true);

  useEffect(() => {
    getAllTickets().then(t => { setTickets(t); setLoading(false); });
  }, []);

  if (loading) return <p className="text-xs text-muted-foreground py-8 text-center">Loading dashboard...</p>;

  const open = tickets.filter(t => t.status === "open").length;
  const reviewing = tickets.filter(t => (t.status as string) === "reviewing").length;
  const verified = tickets.filter(t => (t.status as string) === "verified").length;
  const closed = tickets.filter(t => t.status === "closed").length;
  const needsReply = tickets.filter(t => t.messages[t.messages.length - 1]?.sender === "user").length;
  const totalMessages = tickets.reduce((sum, t) => sum + t.messages.length, 0);

  const stats = [
    { label: "Total Tickets", value: tickets.length, icon: TicketIcon, color: "text-primary" },
    { label: "Open", value: open, icon: Clock, color: "text-yellow-400" },
    { label: "Reviewing", value: reviewing, icon: Eye, color: "text-blue-400" },
    { label: "Verified", value: verified, icon: CheckCircle2, color: "text-green-400" },
    { label: "Closed", value: closed, icon: CheckCircle2, color: "text-muted-foreground" },
    { label: "Needs Reply", value: needsReply, icon: AlertTriangle, color: "text-red-400" },
  ];

  // Recent tickets (last 10)
  const recent = [...tickets].sort((a, b) => new Date(b.createdAt).getTime() - new Date(a.createdAt).getTime()).slice(0, 10);

  // Revenue estimate from ticket specs
  const revenue = tickets.reduce((sum, t) => {
    const price = t.specs.Price || t.specs.Total || "";
    const num = parseFloat(price.replace(/[^0-9.]/g, ""));
    return sum + (isNaN(num) ? 0 : num);
  }, 0);

  return (
    <div className="space-y-4">
      {/* Stats grid */}
      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-3">
        {stats.map(s => (
          <Card key={s.label} className="border-border/50 bg-card/50">
            <CardContent className="p-3 text-center">
              <s.icon className={`h-4 w-4 mx-auto mb-1.5 ${s.color}`} />
              <p className="text-xl font-bold font-display">{s.value}</p>
              <p className="text-[10px] text-muted-foreground">{s.label}</p>
            </CardContent>
          </Card>
        ))}
      </div>

      {/* Revenue + Messages summary */}
      <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
        <Card className="border-border/50 bg-card/50">
          <CardContent className="p-4 flex items-center gap-3">
            <TrendingUp className="h-5 w-5 text-primary shrink-0" />
            <div>
              <p className="text-lg font-bold font-display">₹{revenue.toLocaleString()}</p>
              <p className="text-[10px] text-muted-foreground">Estimated Revenue (from ticket specs)</p>
            </div>
          </CardContent>
        </Card>
        <Card className="border-border/50 bg-card/50">
          <CardContent className="p-4 flex items-center gap-3">
            <MessageCircle className="h-5 w-5 text-primary shrink-0" />
            <div>
              <p className="text-lg font-bold font-display">{totalMessages}</p>
              <p className="text-[10px] text-muted-foreground">Total Messages Across All Tickets</p>
            </div>
          </CardContent>
        </Card>
      </div>

      {/* Recent activity */}
      <Card className="border-border/50 bg-card/50">
        <CardHeader className="pb-2">
          <CardTitle className="font-display text-sm tracking-wider">RECENT TICKETS</CardTitle>
        </CardHeader>
        <CardContent className="p-0">
          <div className="divide-y divide-border/20">
            {recent.length === 0 ? (
              <p className="text-xs text-muted-foreground py-6 text-center">No tickets yet</p>
            ) : recent.map(t => (
              <div key={t.id} className="px-4 py-2.5 flex items-center justify-between">
                <div className="flex items-center gap-2 min-w-0">
                  <span className="font-mono text-[10px] text-muted-foreground">#{t.id.slice(0, 8)}</span>
                  <Badge variant="outline" className="text-[10px]">{t.type}</Badge>
                  <span className="text-xs truncate">{t.email}</span>
                </div>
                <div className="flex items-center gap-2 shrink-0">
                  <Badge className="text-[10px]">{t.status}</Badge>
                  <span className="text-[10px] text-muted-foreground">{new Date(t.createdAt).toLocaleDateString()}</span>
                </div>
              </div>
            ))}
          </div>
        </CardContent>
      </Card>
    </div>
  );
};

export default AdminDashboard;
